...
router.post("/", async (req, res) => {
  try {
    ...
    if (!date || date.length != 10) {
      return res.status(400).json({
        error: "Date is mandatory and should be in format yyyy-mm-dd.",
      });
    }
    ...
    const category = await db.query(
      categoriesQueries.findById(category_id, "fetch-category-finances")
    );
    if (!category.rows[0]) {
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (financeItemQuery.rows[0].user_id !== userQuery.rows[0].id) {
      return res
        .status(401)
        .json({ error: "Finance row doest not belong to users." });
    }
